...
function App() {
  ...
  const [collected, setCollected] = useLocalStorage<Set<string>>(
    "collected",
    new Set(),
  );
  ...
  const handleWheel = (e: Konva.KonvaEventObject<WheelEvent>) => {
    ...
    const mousePointTo = {
      x:
        stage.getPointerPosition()!.x / viewport.scale -
        stage.x() / viewport.scale,
      y:
        stage.getPointerPosition()!.y / viewport.scale -
        stage.y() / viewport.scale,
    };
    setViewport({
      scale: newScale,
      x:
  ...
  const visibleRects = useMemo(() => {
    ...
    const startCol = Math.floor(
      (-viewport.x / currentRectWidth) * viewport.scale,
    );
    const endCol = Math.ceil(
      (window.innerWidth - viewport.x) / (currentRectWidth * viewport.scale),
    );
    const startRow = Math.floor(
      (-viewport.y / currentRectHeight) * viewport.scale,
    );
    const endRow = Math.ceil(
      (window.innerHeight - viewport.y) / (currentRectHeight * viewport.scale),
    );
